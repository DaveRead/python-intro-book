...
If you noticed that ``print'' is not on the list of reserved words give yourself a pat on the back. Beyond Python's reserved words, there are {\bf operations} and {\bf operators} already built into Python. The ``print()'' operation is used frequently. An example operator is the plus sign (+) which may be used to adding numbers. We'll cover these concepts throughout the semester.
...
mistake prone.  You would be smart to write a Java program to handle the
...
When Pythonsays something like:
