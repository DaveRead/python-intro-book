...
\afterverb
%
We say that a value is ``truthy'' or ``falsey'' depending on how Python treats it when used as a boolean value. So 17 is truthy and 0 is falsey. This flexibility can be useful, but there are some subtleties to
it that can be confusing.  You might want to avoid it until
you are sure you know what you are doing. Further, relying on this conversion may make a program harder to understand.
...
Earlier we saw a code segment where we used the \verb"input" and
...
% Updated error message to be clearer that program ended and is not looking for more input.
% Added information about loops in future chapter can be used for asking for input
% until the user enters the correct type.
...
again, or at least end the program gracefully. After you learn about looping in the Iteration chapter you can write code that uses a loop to keep asking for user input until the user enters the desired type of data. 

\section{Short-circuit evaluation of logical expressions}
\index{short circuit}

When Python is processing a logical expression such as 
{\tt x >= 2 and (x / y) > 2}, it evaluates the boolean expressions
...
tabs are invisible and we are used to ignoring them when reading.
...
be 0.  The problem is in line 4, because integer division is being used. The solution is to use floating point division (with the single slash).

\index{floor division}
\index{division!floor}

In general, error messages tell you where the problem was discovered, 
but that may not be where it was caused.
...
\begin{ex}
Rewrite your pay computation (exercise \ref{exCh2HourRate}, page \pageref{exCh2HourRate}) to give the employee 1.5 
...
\begin{ex}\label{exCh3HourRate}
Rewrite your pay program, from above, using {\tt try} and {\tt except} 
